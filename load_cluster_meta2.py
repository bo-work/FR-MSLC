...
# check fea+ex in tls with shot


sys.path.append(".")
from models.gcxyuan.mcre import DeepRe, MoCo, load_model, MLPAE_for_DeepRe, DeepRe_Reload_Emb
...
class KMeansSampleSelector:
    ...
    def fit(self, train_samples, train_gt_label = None):
        ...
        if self.shot_type == 'annote' and train_gt_label is None:
...
def expand_meta(config, train_dataset, val_dataset, logger, shot_type = 'note'):  #shot_type = 'note' 'annote'
    ...
    if config['dataset'] == 'tls23' and config['label_init_type'] == 'kmeans' and config['meta_shot_type']=='note':
    ...
    elif config['dataset'] == 'tls23' and config['label_init_type'] == 'kmeans' and config['meta_shot_type']=='annote':
        optimal_k = config['optimal_k']
        selector = KMeansSampleSelector(n_clusters=optimal_k, shot_type='annote')
        selector.fit(train_fea, train_gt_label)
        # # save kmeans model
        # kmeans_name = '_'.join(['model_kmeans',config['cluster_fea_type'], config['noise_pattern']+str(config['noise_ratio'])+'.pkl'])
        # with open(os.path.join(config['savedir'], kmeans_name), 'wb') as f:
        ...
        meta_data_ex, meta_flabel_ex, meta_gtlabel_ex, kmeans_label_map = selector.select_samples_label(
            config['num_classes'],  n_samples=150, shot_type='annote')
    ...
    elif config['dataset'] == 'ids17c' and config['label_init_type'] == 'kmeans' and config['meta_shot_type']=='note':
        optimal_k = config['optimal_k']
        selector = KMeansSampleSelector(n_clusters=optimal_k, shot_type='note')
        selector.fit(train_fea, train_gt_label)
        # # save kmeans model
    ...
    elif config['dataset'] == 'ids17c' and config['label_init_type'] == 'kmeans' and config[
        'meta_shot_type'] == 'annote':
        ...
        meta_ex_name = '_'.join(
            ['meta_data', config['cluster_fea_type'], config['noise_pattern'] + str(config['noise_ratio']) + '.npz'])
        ...
        kmeans_label_map_name = '_'.join(['model_kmeans_labels_map', config['cluster_fea_type'],
...
for noise_pattern in ['asym']:
    for noise_ratio in [0]:
        ...
        config['reload_file'] = 'model_mcre_checkpoint_' + config['noise_pattern'] + str(config['noise_ratio']) + '_lastep.pth'

        # get metadata from method 1
        expand_meta(config, train_dataset, val_dataset, logger, shot_type=config['meta_shot_type'])
        print('finish')